...
namespace FJASTP{
    ...
    ///@brief a pool of arrays where each array is a fixed size of type T. The way we "allocate" is by using a index into these preallocated arrays. If we need more we just reallocate the vector of the array pointers. This way we dont need to change old pointers and we dont need to reserve a lot of unnecessary nodes.
